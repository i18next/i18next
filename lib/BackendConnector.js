...
var Connector = (function (_EventEmitter) {
  ...
  function Connector(backend, cache, store, services) {
  ...
  _createClass(Connector, [{
    ...
    key: 'loadFromCache',
    value: function loadFromCache(languages) {
      var _this4 = this;

      if (this.cache && this.options.cache && this.options.cache.enabled) {
        this.cache.load(languages, function (err, data) {
          if (err) _this4.logger.error('loading languages ' + languages.join(', ') + ' from cache failed', err);
          if (data) {
            for (var l in data) {
              for (var n in data[l]) {
                if (n === 'i18nStamp') continue;
                var bundle = data[l][n];
                if (bundle) _this4.store.addResourceBundle(l, n, bundle);
              }
            }
          }
        });
